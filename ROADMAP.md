...
2. Raise the remaining shared loader and var-tree behavior to reference-repo parity.
   - Hierarchical `settings.yml` inheritance across nested directories is now implemented.
   - Remaining work:
     - extend validation depth for more nested field models and cross-file references as new domains land
     - keep the runtime field model, example var files, and validation logic aligned as the repo grows

3. Build the `tenants` domain for tenant and chassis-partition lifecycle.
...
4. Build the `software_lifecycle` domain for software and image workflows.
...
5. Build the `observability` domain for diagnostics, exports, and non-config runtime collection tasks.
   - Completion target: `runtime+validation`
   - Module coverage:
     - `f5os_device_info`
     - `f5os_qkview`
...
6. Add cross-platform modeling rules before implementation spreads.
...
7. Add validation contracts per domain before claiming parity.
...
8. Add the supporting documentation set after the first domain skeletons exist.
   - `docs/index.md`
...
9. Add repository tooling and developer ergonomics to match the reference repo's standard.
